...
//
// `default_service` / `default_path` are overridden at the call site
// (`pass_startup_args_to_existing_instance`) on the builder via `.destination(well_known_name())` /
// `.path(application_service_path())`, so the constants here are not actually used; but to avoid
// misleading future callers that use the default directly via `Proxy::new`, they still point at the
// OSS default `dev.zap.Zap`.
#[proxy(
