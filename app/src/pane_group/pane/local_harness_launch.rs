...
use crate::ai::agent_sdk::driver::harness::{
    HarnessKind, harness_kind, remove_claude_externally_managed_listener_env_vars,
};
...
pub(super) async fn prepare_local_harness_child_launch(
    ...
    let command = match harness {
        ...
        Harness::Codex => {
            ...
            // auth/session state. Never run shared Codex environment prep
            // here: it can seed OPENAI_API_KEY into ~/.codex/auth.json and
            // rewrite ~/.codex/config.toml globally.
    ...
    // longer goes through the cloud `create_agent_task` mutation; it generates a
    // UUID v4 locally as the task_id. The `local_child_task_config(harness, agent_name)`
    // call's result is no longer used.
