//! Lazy restoration of off-tree child agent panes (upstream #10371).
//!
//! Zap: upstream also seeds remote children from the server's
//! `?ancestor_run_id=` listing and hydrates viewer / remote-worker children
//! from cloud task data (`child_agent/hydration.rs`). Zap's orchestration is
//! local-only (local launcher + BYOP agent loop), so every child here is
//! restored from its local transcript into a local off-tree pane.
...
impl PaneGroup {
    ...
    ///
    /// Zap: upstream dispatches viewer-side (`is_viewing_shared_session`) and
    /// remote-worker (`is_remote_child`) children to cloud hydration first.
    /// Zap never creates either kind (no Warp cloud), so every child takes the
    /// local branch below and its persisted transcript is restored in place.
    pub(in crate::pane_group) fn create_hidden_child_agent_pane(
        ...
        // context (`launch_local_harness_child`), so only they get it back on
        // restore. A local Oz child's task id is the synthetic run id
        // `launch_local_oz_child` assigns (its own conversation id); applying
        // it would mark the restored child's requests as an ambient-agent run
        // (see `is_ambient_agent` in `ai::agent::api`), which the child never
        // was when it ran.
