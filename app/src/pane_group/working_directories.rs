...
impl WorkingDirectoriesModel {
    ...
    pub fn refresh_working_directories_for_pane_group(
        ...
        let mut remote_terminal_cwds: Vec<(EntityId, warp_util::remote_path::RemotePath)> =
            Vec::new();
        ...
        let mut remote_editor_paths: Vec<(EntityId, warp_util::remote_path::RemotePath)> =
            Vec::new();
        ...
        for (_view_id, remote_path) in &remote_editor_paths {
                ...
                DetectedRepositories::as_ref(ctx).get_root_for_path(&remote_key)
            {
                ...
                new_remote_display_roots.push(LocalOrRemotePath::Remote(
                    warp_util::remote_path::RemotePath::new(remote_path.host_id.clone(), parent),
                ));
            }
        }
