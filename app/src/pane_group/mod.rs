...
pub enum Event {
    ...
    OpenAddRulePane,
    // Zap Wave 7-3: the `OpenEnvironmentManagementPane` event was physically
    ...
    /// Zap: emitted when Ctrl/Cmd+clicking a file path in remote SSH session
    /// output in the terminal; the workspace opens the remote file in the
    /// editor via the buffer-sync protocol.
...
pub struct PaneGroup {
    ...
    // `pane_with_open_agent_assisted_environment_modal` were physically removed
...
impl PaneGroup {
    ...
    fn restore_pane_leaf(
        ...
        let result = match leaf.contents {
            ...
            LeafContents::Sftp { .. } => {
                // The SFTP browser pane is not persisted; the remote file system
    ...
    /// Find the visible terminal pane whose PTY session UUID matches (warposs://pane deep link).
    ...
    fn new_internal(
        ...
        let mut pane_group = Self {
            ...
            // Zap Wave 7-3: the pane-level modal tracking fields in the
            // ambient-agent UI subsystem were physically removed along with the UI.
    ...
    fn discard_pane(&mut self, pane_id: PaneId, ctx: &mut ViewContext<Self>) {
        // Preserve local child-agent ownership when a non-child pane closes.
        if !self.is_child_agent_pane(pane_id) {
    ...
    pub fn close_pane(&mut self, pane_id: PaneId, ctx: &mut ViewContext<Self>) {
        ...
        //
        // Zap: this fork now carries upstream's off-tree child-agent model
        // (#10327 swap-in-place + #10371 lazy restoration, see
        // `child_agent/restoration.rs`): every reveal re-hosts the pane through
        // `ensure_hidden_child_agent_pane_for_conversation` plus
        // `swap_active_pane_to_conversation` / `unhide_child_agent_pane_for_split_off`,
        // so closing must leave the pane neither in the tree nor in
        // `hidden_panes`. Do not bring back the pre-#10327 `hide_pane_for_child_agent`
        // re-hide here: nothing reveals `HiddenPaneReason::ChildAgent` panes anymore.
        if self.is_child_agent_pane(pane_id) {
        ...
        if FeatureFlag::UndoClosedPanes.is_enabled() {
            ...
            // Zap Wave 7-3: cleanup of the pane-level modal tracking fields in the
            // ambient-agent UI subsystem was physically removed along with the UI.
    ...
    fn replace_code_pane_with_file_pane(
        ...
        file_pane.file_view(ctx).update(ctx, |view, ctx| {
            ...
            view.open(LocalOrRemotePath::Local(path), session, ctx);
        });
        ...
        match event {
            ...
            PaneEvent::RemoteRepoNavigated { remote_path } => {
                ctx.emit(Event::RemoteRepoNavigated {
                    host_id: remote_path.host_id.clone(),
                    indexed_path: remote_path.path.to_string(),
                });
            }
        }
    }
    ...
    fn create_conversation_viewer(
        ...
        // Insert the conversation-ended tombstone (includes Open in Zap on WASM).
        terminal_view.update(ctx, |view, ctx| {
            view.insert_conversation_ended_tombstone(ctx);
        });
    ...
    pub fn reattach_panes(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let pane_ids = self.pane_contents.keys().copied().collect::<Vec<_>>();
        for pane_id in pane_ids {
    ...
    pub fn close_overlays(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        // Zap: removed terminal_with_open_share_block_modal reset (the field no longer exists)
        ctx.notify();
    }
...
impl View for PaneGroup {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        // agent-assisted environment modal was physically removed along with the ambient-agent UI subsystem.
