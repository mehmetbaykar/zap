...
pub struct TerminalModel {
    ...
    /// A viewer attaches after the local model has already reached its prompt state. Accept the
    /// first replayed Precmd so the viewer can hydrate the sharer's prompt metadata without
    /// weakening repeated-Precmd rejection for local or sharing sessions.
    accept_next_viewer_precmd_replay: bool,
...
impl TerminalModel {
    ...
    fn new_internal(
        ...
        let accept_next_viewer_precmd_replay = shared_session_status.is_viewer();
        ...
        Self {
            ...
            accept_next_viewer_precmd_replay,
    ...
    pub fn set_shared_session_status(&mut self, shared_session_status: SharedSessionStatus) {
        if !self.shared_session_status.is_viewer() && shared_session_status.is_viewer() {
            self.accept_next_viewer_precmd_replay = true;
        } else if !shared_session_status.is_viewer() {
            self.accept_next_viewer_precmd_replay = false;
        }
...
impl ansi::Handler for TerminalModel {
    ...
    fn prompt_only_precmd(&mut self, data: PromptMetadata) {
        ...
        let should_apply_viewer_replay = self.accept_next_viewer_precmd_replay
            && self.shared_session_status().is_viewer()
            && matches!(
                transition.action,
                LifecycleAction::Ignore(IgnoreReason::RepeatedPrecmd)
            );
        if matches!(transition.action, LifecycleAction::ApplyPrecmd) || should_apply_viewer_replay {
