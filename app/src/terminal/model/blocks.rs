...
impl BlockList {
    ...
    ///
    /// Returns whether the flag was actually applied. It is not applied when the
    /// rich-content view has no position yet (not mounted), and callers that
    /// memoize the last written value MUST NOT record one on a `false` return:
    /// the item later mounts with `is_agent_transcript_user_query: false`, so a
    /// memo written here would suppress the retry and the segment would never
    /// become a Cmd-Up stop.
