...
impl QueuedPromptsPanelView {
    ...
    /// Whether the header shows the "⏎ to send" hint: Enter would send and no row is in inline
    /// edit mode.
    ...
    /// Updates each row's "send now" button. When a long-running-command subagent (the "full
    /// terminal use agent") is in control, the enabled tooltip explains that send-now targets
    /// that subagent. Otherwise it is enabled with the default "Send now" tooltip.
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            QueuedPromptsPanelAction::SendNow(query_id) => {
                ...
                // fires; the host removes the fired row afterward.
                let row = QueuedQueryModel::as_ref(ctx)
                    .queue(conv_id)
                    .iter()
                    .find(|row| row.id() == query_id)
