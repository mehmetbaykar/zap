...
impl TerminalView {
    ...
    ///
    /// Always `None` in this fork: `AIConversation` no longer carries server-fetched
    /// conversation metadata (cloud-only concept, removed with the rest of the server/GraphQL
    /// restore path), so there is no local data source for restored cloud transcripts.
