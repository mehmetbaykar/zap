...
impl super::TerminalView {
    ...
    ///
    /// When `local_tty` is not enabled / on wasm / the `SshRemoteServer` feature flag is off,
    /// always returns `false`, i.e. keeps local behavior completely unchanged.
    ...
    /// Zap: gets the directory-listing validation context for a given cwd of a remote session.
    ///
    /// On a cache hit, returns `Remote(Some(..))` directly; on a miss, asynchronously issues a
    /// daemon `ListDirectory` RPC to fetch the directory listing, returns `Remote(None)` (no
    /// highlight) for this round, and once the fetch completes writes the cache and calls
    /// `ctx.notify()` to trigger a re-render that lights up the link.
    ///
    /// The cache stays bounded: when fetching a new cwd, all old entries are cleared, keeping only
    /// the current cwd.
    #[cfg(all(
        feature = "local_tty",
        feature = "local_fs",
    ...
    fn remote_dir_listing_context(
        ...
        // The cache is indexed by the composite (session, cwd) key, to avoid identical paths on
        // different hosts crosstalking with each other.
        let cache_key = (session_id, cwd_path.clone());

        // On a cache hit (ready or fetching), return directly.
        ...
        // Fetch a new cwd: when over the capacity limit, evict the oldest entry FIFO by insertion
        // order, then insert a `None` placeholder (marking it as fetching). `MAX_ENTRIES` of 8 is
        // enough to cover the few working directories the user commonly switches between in the
        // terminal, avoiding an RPC every time they switch back.
        ...
        ctx.spawn(
            ...
            move |me, result, ctx| {
                ...
                // During the fetch the user may have switched cwd / cleared the cache; only write
                ...
                match result {
                    Ok(resp) => match resp.result {
                        Some(list_directory_response::Result::Success(success)) => {
                            ...
                            // The listing has arrived; trigger a re-render so links are rescanned
                            // and lit up.
                            ctx.notify();
                        }
                        Some(list_directory_response::Result::Error(err)) => {
                            ...
                            // Fetch failed: remove the placeholder, it will be retried on the next
                            // hover.
                    ...
                    Err(err) => {
                        log::warn!("remote ListDirectory RPC errored {cwd_for_store:?}: {err}");
                        me.remote_dir_listing_cache.shift_remove(&key_for_store);
                    }
                }
            },
    ...
    fn scan_for_file_path(
        ...
        // Zap: determine whether the session that owns the hovered block is a remote-server
        // session. A remote session's files are not on the local disk, so we need to use
        // `LinkValidationContext::Remote` carrying the real directory listing fetched from the
        // daemon to do precise validation.
        ...
        //
        // Zap: a remote session's block `pwd()` is the remote cwd reported by shell-integration,
        // and joining it yields the correct remote absolute path, so remote blocks also
        ...
        // Zap: remote sessions validate precisely using the cached cwd directory listing; local
        // sessions stay `Local`.
        ...
        match pwd_to_scan_for {
            ...
            Some(path) if matches!(from_editor, TerminalEditor::No) => {
                ...
                // Use the hovered block's own launch data, to avoid parsing the path with the
                // wrong shell rules across sessions/hosts/WSL.
