        ...
        for (block_id, task_id) in targets {
            let should_restore = {
                ...
                model
                    ...
                    .is_some_and(|block| {
                        // The restored command block is already complete, so we can't rely on the
                        // live long-task predicate; use the persisted metadata to confirm precisely
                        // that it belongs to the current conversation's CLI subagent task.
    ...
    pub fn create_and_insert_ai_block(
        ...
        let restored_block_view_handle = ctx.add_typed_action_view(|ctx| {
            AIBlock::new(
                ...
                Some(self.ambient_agent_view_model.clone()),
