...
impl<S> TerminalManager<S> {
        ...
        BoxManager: FnOnce(Self) -> Box<dyn TerminalManagerTrait> + 'static,
    {
        ...
        // Shared-session creation is an inert compatibility input in Zap. Keep the
        // constructor shape shared with WarpUI/TUI callers without starting network state.
        match is_shared_session_creator {
            IsSharedSessionCreator::Yes { .. } => {
                log::debug!("Ignoring shared-session creation request in Zap");
            }
