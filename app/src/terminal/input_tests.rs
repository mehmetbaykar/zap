...
fn send_now_event_submits_through_active_pane_and_preserves_draft() {
    ...
    // clobbering a draft the user has typed locally.
    App::test((), |mut app| async move {
        initialize_app(&mut app);
        ...
        let submission_count = Rc::new(RefCell::new(0));
        let submission_count_for_subscription = submission_count.clone();
        app.update(|ctx| {
            ctx.subscribe_to_view(&input, move |_, event: &super::Event, _| {
                if matches!(event, super::Event::ExecuteAIQuery) {
                    *submission_count_for_subscription.borrow_mut() += 1;
                }
            });
        });
...
fn test_open_slash_command_expands_tilde() {
    App::test((), |mut app| async move {
        ...
        let file_path = test_home.path().join("tilde_test_file.txt");
        std::fs::write(&file_path, "tilde expansion fixture").unwrap();

        let session_id: SessionId = 1.into();
        let session_info = SessionInfo::new_for_test()
            .with_id(session_id)
            .with_home_dir(test_home.path().to_string_lossy().into_owned());
        let terminal = add_window_with_bootstrapped_terminal(
            &mut app,
            None, /* history_file_commands */
            Some(session_info),
        ...
        input.update(&mut app, |input, ctx| {
            input.editor.update(ctx, |editor, ctx| {
                editor.set_buffer_text("/open-file ~/tilde_test_file.txt", ctx)
            });
        });
