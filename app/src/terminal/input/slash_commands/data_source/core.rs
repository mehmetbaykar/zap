...
///
/// These do not depend on GUI-only concepts such as the agent view; they are computed once per
/// recompute and shared by both surfaces.
    ...
    T: Entity<Event = UpdatedActiveCommands>,
{
    ...
    // Recompute when the active conversation switches so conversation commands update on
    // navigation.
    ctx.subscribe_to_model(
...
/// such as agent view, recomputation, and event emission lives on the wrapping surface types.
...
pub trait SlashCommandDataSource {
    ...
    ///
    /// Surfaces add their own bits (agent view vs. terminal view and active conversation) on top
    /// of this baseline.
    ...
    /// vector when skills are globally unavailable.
