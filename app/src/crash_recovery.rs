...
struct CrashRecoveryProcess {
    ...
    /// The recent render success/failure sequence for the current window; when there are too many
    /// consecutive failures, write to the local log and then exit.
    ...
    fn handle_draw_frame_error(&mut self, window_id: WindowId) {
        ...
        *num_errors += 1;
        ...
        if *num_errors >= NUM_DRAW_ERRORS_BEFORE_EXITING {
            ...
            // Stop the local crash reporting process before exiting, to avoid leaving a background
