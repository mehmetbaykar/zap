...
use crate::settings::{
    ...
    respect_system_theme, AIFontName, AISettings, AppEditorSettings, CodeSettings, CursorBlink,
    ...
    InputSettings, InputSettingsChangedEvent, MarkdownHeadingH1Scale, MarkdownHeadingH2Scale,
    ...
    ThemeSettings, UiFontName, UseSystemTheme, UseThinStrokes, DEFAULT_MONOSPACE_FONT_NAME,
...
impl AppearanceSettingsPageView {
    ...
    /// Handles events from Markdown heading font-size multiplier editors.
    ...
    fn set_markdown_heading_scale(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        // Fallback for cells that failed to parse, or parsed and clamped to the current value
        // (where set_value does not emit an event): restore editors to the actual setting value.
        self.refresh_markdown_heading_scale_editors(ctx);
    }

    /// Resets Markdown heading font-size multipliers to defaults.
...
/// Settings widget for Markdown heading font-size multipliers.
impl SettingsWidget for MarkdownHeadingScaleWidget {
    ...
    fn render(
        ...
        // Title row: label and reset button on one line.
