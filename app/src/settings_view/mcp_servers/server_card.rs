...
impl StatusColor {
    fn to_color(&self, appearance: &Appearance) -> ColorU {
        match self {
            ...
            // The status indicator dot uses the raw ANSI colors to avoid ansi_fg_* blending half the
            // saturation into the foreground, which produces a muddy "grey-green / grey-yellow" look
            // in low-saturation-foreground themes like One Dark / Tokyo Night.
