//! Code settings page: after Zap retired the full LSP stack and the persistent workspace
//! history, this page is left with just a few local toggles related to "Editor and Code Review".
//!
//! Historically this also hosted the LSP management subpage and codebase indexing, but both were
//! retired; `Code` is no longer an umbrella in the sidebar (there is no second subpage to attach),
//! and is now a single-level Page. The page renders exactly this group of toggles.
