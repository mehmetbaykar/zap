...
///
/// Folds the parent feature flag and the user's dedicated per-feature AI toggle.
///
...
impl GitDialog {
    pub fn new_for_commit(
        ...
        let (confirm_button, cancel_button, close_button) =
            Self::build_dialog_buttons("Confirm".to_string(), None, ctx);
    ...
    fn handle_diff_state_event(
        ...
        match result {
            GitOpResult::CommitChainCompleted(result) => {
                ...
                // Unified completion path (toast + close) for both
