...
use crate::ai::agent_providers::oneshot::resolve_active_ai_oneshot;
...
impl LocalDiffStateModel {
    ...
    pub fn git_commit_chain(
        ...
        let byop_config = (matches!(mode, CommitChainMode::CommitAndCreatePr)
            && autogenerate_pr_content)
            .then(|| resolve_active_ai_oneshot(ctx, None))
            .flatten();
    ...
    pub fn create_pr(
        ...
        let byop_config = autogenerate_content
            .then(|| resolve_active_ai_oneshot(ctx, None))
            .flatten();
        let path_future = Self::interactive_path_future(ctx);
        ctx.spawn(
