...
pub enum DiffStateModelEvent {
    ...
    /// A locally generated BYOP commit message arrived for a remote diff.
    /// `Ok` carries the message and `Err` the error string.
...
impl DiffStateModel {
    ...
    pub(crate) fn create_pr(
        ...
        match self {
            ...
            Self::Remote(remote) => remote.update(ctx, |remote, ctx| remote.create_pr(branch, ctx)),
        }
    }
