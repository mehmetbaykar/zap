...
impl FileNotebookView {
    ...
    ///
    /// Jupyter notebooks are rendered via `reset_with_ipynb` when the feature
    /// flag is enabled; otherwise content is treated as Markdown.
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            FileNotebookAction::OpenInEditor => {
                // Jupyter notebooks should open as raw code (JSON) rather than
                // re-resolving through the markdown-viewer preference path.
            ...
            FileNotebookAction::ContextMenu(action) => {
                if matches!(action, ContextMenuAction::Open(_)) {
                    ...
                    let copy_file_path = self.file_state.local_path().map(|p| p.display().to_string());
