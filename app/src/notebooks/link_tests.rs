...
fn test_open_markdown_file_respects_disabled_viewer_preference() {
    ...
    App::test((), |mut app| async move {
        ...
        let events = events.lock();
        // Zap: with the Markdown Viewer preference disabled and the default
        // editor choice (SystemDefault), the fork treats the OS default
        // handler as a dangerous target and reveals the file in the file
        // manager instead of emitting an open event (upstream emits
        // OpenFileWithTarget { target: SystemDefault } here).
        assert!(
            events.is_empty(),
            "expected no link events for a SystemDefault markdown open, got {events:?}"
        );
    });
}
