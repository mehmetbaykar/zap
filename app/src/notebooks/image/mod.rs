...
impl ImageViewerView {
    ...
    pub fn open_remote(
        ...
        // Use host_id:path as a stable asset id so identical remote images reuse cache entries,
        // while different hosts or paths stay isolated.
...
impl View for ImageViewerView {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let body: Box<dyn Element> = match &self.source {
            ...
            // Remote images have no source while bytes are being fetched, so show a loading state.
