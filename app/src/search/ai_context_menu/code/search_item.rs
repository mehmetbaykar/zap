...
impl SearchItem for CodeSearchItem {
    ...
    fn render_item(
        ...
        let mut path_display = self.code_symbol.file_path.to_string_lossy().to_string();
        ...
        // Matching concatenates the type prefix and name, but display inserts a space.
        let symbol_highlights: Vec<usize> = self
            .match_result
            .matched_indices
            .iter()
            .filter_map(|&i| {
                ...
                if !symbol_truncated || adjusted_i < symbol_name.len().saturating_sub(3) {
                    Some(adjusted_i)
                } else {
                    None
                }
    ...
    fn accept_result(&self) -> Self::Action {
        // Keep the upstream textual reference understood by both terminal and agent inputs.
