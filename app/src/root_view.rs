...
///
/// We use `ChannelState::app_id().application_name()` so that the OSS build shows `Zap`, while
/// upstream channels like Stable/Preview/Dev still show their respective `Zap` / `WarpPreview` /
/// `WarpDev`, avoiding hard-coding the string in many places across the fork (the Windows Task
/// Manager groups processes by window title, and hard-coding `"Zap"` would make Zap show up as
/// `Zap(N)` in the Task Manager).
///
/// Note: after the window is created, `Workspace::update_window_title()` overwrites this value with
/// the tab title on every tab switch/rename, so this function only determines the initial title when
/// the window has just opened and no tab is attached yet.
...
impl RootView {
    pub fn new(
        ...
        // The original `InitialLoadCompleted` event was used to call `apply_onboarding_settings`
        // after cloud preferences sync completed; in the localized scenario the onboarding settings
        // are applied locally directly.
    ...
    /// If the user has completed local onboarding before entering the terminal, fill in the local
    /// `is_onboarded` flag immediately once the auth facade enters its "available" state.
    ///
    /// This logic runs on every `AuthComplete`, so it also covers the "skip login first, then enter
    /// the authenticated state from another entry point" path; the whole process only updates the
    /// local auth facade and does no server-side sync.
    ...
    fn handle_auth_manager_event(&mut self, event: &AuthManagerEvent, ctx: &mut ViewContext<Self>) {
        ...
        match event {
            AuthManagerEvent::AuthComplete => {
                // If onboarding finished before entering the auth-complete state, fill in the local
                // `is_onboarded` bit here, to avoid later code still acting on the "onboarding
                // incomplete" branch.
                ...
                UserAuthenticationError::UserAccountDisabled => {
                    cfg_if! {
                        if #[cfg(target_family = "wasm")] {
                            ...
                            // Zap has removed the log_out UI entry point; native no longer forces a
                            // logout.
    ...
    fn handle_auth_override_warning_modal_event(
        ...
        match event {
            AuthOverrideWarningModalEvent::Close => {
                // Zap has removed the log_out entry point; closing no longer triggers a logout.
            }
    ...
    /// Zap (localization, Phase 5): the original `handle_preferences_syncer_event` applied
    /// onboarding settings after the cloud preferences sync's initial load completed, and was
    /// physically deleted along with the syncer. Onboarding settings are now applied directly when
    /// onboarding completes, with no need to defer until after cloud sync.
...
impl AuthOnboardingState {
    fn complete_auth_and_create_workspace(&mut self, ctx: &mut ViewContext<RootView>) {
        ...
        // The local `is_onboarded` flag is filled in by
        // `RootView::finalize_local_onboarding_after_auth` on `AuthComplete`.
