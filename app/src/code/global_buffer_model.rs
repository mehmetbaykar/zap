...
use super::buffer_location::{BufferLocation, RemotePath, SyncClock};
...
impl PendingEditBatch {
    ...
    fn flush(self, client: &remote_server::client::RemoteServerClient, path: &str) {
        ...
        if let Err(error) = client.send_buffer_edit(
            path.to_string(),
            self.expected_server_version,
            self.latest_client_version.as_u64(),
            self.edits,
        ) {
            log::error!("Failed to send batched remote buffer edits for {path}: {error}");
        }
    }
...
impl GlobalBufferModel {
    ...
    /// Client-only subscription to remote-buffer pushes. This is registered
    /// explicitly because the remote daemon also owns a `GlobalBufferModel`
    /// but does not register `RemoteServerManager`.
    ...
    pub fn subscribe_to_remote_server_manager(ctx: &mut ModelContext<Self>) {
        ...
        ctx.subscribe_to_model(&manager, |me, _, event, ctx| match event {
            RemoteServerManagerEvent::BufferUpdated {
                ...
                let char_edits = edits
                    .iter()
                    .map(|edit| CharOffsetEdit {
                        start: CharOffset::from(
                            usize::try_from(edit.start_offset).unwrap_or(usize::MAX),
                        ),
                        end: CharOffset::from(
                            usize::try_from(edit.end_offset).unwrap_or(usize::MAX),
                        ),
    ...
    /// Actively close a buffer and remove it from the global buffer map.
    /// Remote buffers also notify the daemon so its in-memory buffer is released.
    ...
    pub fn close_buffer(&mut self, file_id: FileId, ctx: &mut ModelContext<Self>) {
        ...
        if let Some(state) = self.buffers.get(&file_id)
            && let BufferSource::Remote { remote_path, .. } = &state.source
        {
    ...
    fn find_remote_file_id(&self, host_id: &HostId, path: &str) -> Option<FileId> {
        ...
        let location = BufferLocation::Remote(RemotePath::new(host_id.clone(), std_path));
        self.location_to_id.get_by_left(&location).copied()
    }

    /// Returns whether a buffer is backed by an SSH remote host.
    #[cfg(feature = "local_tty")]
    ...
    pub fn resolve_conflict(
        ...
        // Keep the buffer's current version and update base_content_version only
        // after FileModel confirms the save, preserving conflict detection if
        // the async disk write fails.
    ...
    /// Re-open an existing remote buffer by sending `OpenBuffer` to the server.
    ///
