...
impl CustomEndpointSettingsModel {
    ...
    fn sync_or_migrate(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        // Zap has no cloud preferences sync, so there is no initial load to wait for:
        // legacy endpoints live only in local secure storage and are available immediately.
