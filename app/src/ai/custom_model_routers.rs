...
/// The shared `config_key` namespace prefix for local custom model routers.
...
/// Returns whether an id is the `config_key`/`LLMId` of a custom model router.
pub fn is_custom_router_id(id: &str) -> bool {
    id.starts_with(CUSTOM_ROUTER_PREFIX)
}

/// Returns whether an id is the `config_key` of a local YAML-authored router.
