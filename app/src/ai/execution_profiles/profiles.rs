...
enum ProfileSource {
    /// Existing per-profile local object-store entries used by GUI rollback builds.
    LegacyCloudObjects,
...
pub struct AIExecutionProfilesModel {
    ...
    /// Maps stable profile IDs to legacy object-store IDs for the rollback backend.
...
impl AIExecutionProfilesModel {
    ...
    pub(crate) fn new(launch_mode: &LaunchMode, ctx: &mut ModelContext<Self>) -> Self {
                ...
                .is_value_explicitly_set()
        {
            ...
            profiles.insert(
                ...
                super::AIExecutionProfile::create_default_from_legacy_settings(ctx),
            );
        ...
        cfg_if::cfg_if! {
            if #[cfg(feature = "agent_mode_evals")] {
                ...
                let (
                    ...
                    // Legacy launches reconstruct their local ID mappings from the
                    // objects already restored into the local object store.
                    let object_store_model = ObjectStoreModel::handle(ctx).as_ref(ctx);
                    let all_profile_objects: Vec<&super::AIExecutionProfileObject> =
                        object_store_model
                            .get_all_objects_of_type::<GenericStringObjectId, AIExecutionProfileObjectModel>()
                            .collect();

                    let default_profile_object: Option<&super::AIExecutionProfileObject> =
                        all_profile_objects
                            .iter()
                            .find(|obj| obj.model().string_model.is_default_profile)
                            .copied();

                    let mut profile_id_to_sync_id: HashMap<ExecutionProfileId, SyncId> =
                        HashMap::new();
                    let active_profiles_per_session: HashMap<EntityId, ExecutionProfileId> =
                        HashMap::new();
        ...
        if !uses_file_backed_profiles {
            ...
            // Zap: `CloudPreferencesSyncer` is deleted, so there is no cloud
            // preference load to wait for. `ObjectStoreModel` restores every stored
            // object from SQLite before this model is registered, so the one-time
            // import runs here directly.
            model.migrate_settings_profiles(ctx);
            // The `AISettings` subscription cannot deliver to an unregistered model,
            // so refresh the snapshot the settings-backed reads use.
            model.last_settings_profiles =
    ...
    /// Materializes the locally stored execution profiles into the file-backed collection.
    ///
    /// Zap: upstream gated this on `CloudPreferencesSyncerEvent::InitialLoadCompleted`
    /// ("wait until cloud preferences finished loading"). That syncer is deleted here,
    /// so the gate collapses to "the local object store has loaded": `ObjectStoreModel`
    /// restores every stored object from SQLite before this model is registered, so the
    /// one-time import runs straight from `Self::new`. The object store is only read
    /// here; it is never written back.
    pub(crate) fn migrate_settings_profiles(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        if AISettings::as_ref(ctx)
            ...
            .is_value_explicitly_set()
        {
            // The collection is already authoritative.
            self.preserve_profile_onboarding_overrides = true;
            return;
        }

        let legacy_profile_objects = ObjectStoreModel::as_ref(ctx)
            .get_all_objects_of_type::<GenericStringObjectId, AIExecutionProfileObjectModel>()
            .collect::<Vec<_>>();

        let mut legacy_profiles = legacy_profile_objects
            .into_iter()
            .map(|profile| {
                let id = if profile.model().string_model.is_default_profile {
                    ExecutionProfileId::default_profile()
                } else {
                    ExecutionProfileId::generated()
                };
        ...
        let profiles = if legacy_profiles.is_empty() {
            ...
            let profile = AIExecutionProfile::create_default_from_legacy_settings(ctx);
            let mut profiles = ExecutionProfilesConfig::default();
            ...
            if !legacy_profiles.iter().any(|(id, _)| id.is_default()) {
                legacy_profiles.insert(
                    ...
                    (
                        ExecutionProfileId::default_profile(),
                        AIExecutionProfile::create_default_from_legacy_settings(ctx),
                    ),
                );
            }
