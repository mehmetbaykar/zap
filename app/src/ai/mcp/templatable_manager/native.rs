...
use mcp::oauth::{
    ...
    TEMPLATABLE_MCP_CREDENTIALS_KEY, load_credentials_from_secure_storage, write_to_secure_storage,
};
...
use crate::ai::mcp::templatable_manager::utils::{query_resources_for, query_tools_for};
use crate::ai::mcp::{
    Author, FileBasedMCPManager, JsonTemplate, MCPGalleryManager, MCPServer, MCPServerObject,
    MCPServerUpdate, ParsedTemplatableMCPServerResult, StaticEnvVar, TemplatableMCPServer,
    TemplatableMCPServerInstallation, TransportType, logs,
};
...
async fn spawn_server(
    ...
    let service = match transport_type {
        TransportType::CLIServer(cli_server) => {
            ...
            // rmcp::transport::TokioChildProcess::builder mandates a tokio::process::Command and
            // cannot be swapped for the command::r#async::Command wrapper; instead manually set CREATE_NO_WINDOW below
            // to keep Windows from flashing a window.
    ...
    Ok(TemplatableMCPServerInfo::new(
        server_name,
        service,
        resources,
        tools,
...
async fn determine_transport(
    ...
    match send_initialize_request(url, headers, None).await? {
        ...
        StatusCode::UNAUTHORIZED => {
            ...
            let mut auth_context = auth_context.ok_or_else(|| {
                rmcp::RmcpError::transport_creation::<ReqwestHttpTransport>(
                    "MCP OAuth is enabled, but no authentication context is available.".to_string(),
                )
            })?;
            let authenticated = auth_context.authenticated.take();
            ...
            if transport.is_ok()
                && did_require_login
                && let Some(authenticated) = authenticated
                && let Err(err) = authenticated(server_name).await
            {
