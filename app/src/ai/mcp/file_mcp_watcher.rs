...
impl FileMCPWatcher {
    ...
    /// then emits the matching [`FileMCPWatcherEvent`]. Any in-flight parse for the
    /// same `(path, provider)` is aborted first so stale results cannot land late.
