...
impl MCPGalleryManager {
    pub fn new(_ctx: &mut ModelContext<Self>) -> Self {
        // Zap (localization, Phase 2d-2): this originally subscribed to `UpdateManager`'s `MCPGalleryUpdated` event
        // to distribute gallery items after a cloud fetch. After localization, the cloud object fetch/fan-in is removed,
        // and this Phase keeps the gallery empty and unsubscribes ——
        // the gallery is always empty locally and is rendered as an empty canvas by `MCPServersListPageView`; local MCP goes through `file_based_manager`
        // reading `~/.warp/mcp.json` and `~/.claude/...`, unaffected.
        Self {
