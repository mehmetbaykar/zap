...
pub(crate) fn convert_tool_call_result_to_input(
    ...
    match tool_call_result.result.as_ref() {
            ...
            let command_output_result = match &result.result {
                Some(api::run_shell_command_result::Result::CommandFinished(finished)) => {
                    RequestCommandOutputResult::Completed {
                        ...
                        // start/finish timestamps; keep the native fields but leave unset.
                        start_ts: None,
                        completed_ts: None,
                    }
                }
        ...
        Some(ToolCallResultType::WriteToLongRunningShellCommand(result)) => {
            let write_result = match &result.result {
                    ...
                    Some(api::write_to_long_running_shell_command_result::Result::CommandFinished(
                        ...
                        output: finished.output.clone(),
                        exit_code: ExitCode::from(finished.exit_code),
                        // ShellCommandFinished no longer carries start/finish timestamps.
                        start_ts: None,
                        completed_ts: None,
                    },
        ...
        Some(ToolCallResultType::UploadFileArtifact(_)) => {
            // Upload artifact has been physically excised; the cloud result is simply discarded
            let _ = tool_call_id;
            let _ = task_id;
            let _ = context;
            None
        ...
        | Some(ToolCallResultType::RequestComputerUseResult(_)) => {
            // Computer Use has been removed; these two kinds of results are simply ignored when encountered in history.
            None
        }
