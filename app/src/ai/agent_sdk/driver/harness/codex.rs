...
/// Codex sees the same skills the local harness loads from `WARP_SKILL_DIRS`.
///
...
/// on its own. A published skill overrides any existing entry with the same
/// name (see `skill_dirs_publish::publish_skill`), with the
