...
impl RuleEditorView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        // Zap (localization, Phase 2d-1): the original NetworkStatus subscription was used to redraw on online/offline status changes
        // (driving the `is_delete_allowed` predicate); after localization that predicate is always true, so the subscription is dead code and is removed.
...
impl View for RuleEditorView {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        // Zap (localization, Phase 2d-1): the original `is_delete_allowed` depended on network-online + server_id;
        // after localization, an existing rule being edited can always be deleted, so the predicate is removed.
