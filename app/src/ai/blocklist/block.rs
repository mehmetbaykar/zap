...
impl AIBlock {
    ...
    pub fn new(
        ...
        // Zap: this originally subscribed to GetRelevantFilesController's Success event to do a
        // `ctx.notify` refreshing the UI after RAG completed. That controller was decommissioned along with the outline rollout,
        // so the subscription point is removed in tandem.
        ...
        // Zap (Phase 3c A1): removed the subscription to `AIRequestUsageModelEvent::RequestBonusRefunded`.
        // After localization that event is never emitted (there is no `provide_negative_feedback_response`
        // RPC call), so the subscription has become permanently idle dead code. The `request_refunded_count` field is kept
        // but is always its initial value None.
        ...
        for (id, content) in comment_data {
            ...
            ctx.subscribe_to_view(&state.rich_text_editor, |me, view, event, ctx| {
                if matches!(event, EditorViewEvent::TextSelectionChanged)
                    && let Some(selected_text) = view.as_ref(ctx).selected_text(ctx)
                {
    ...
    fn handle_complete_output(&mut self, output: &AIAgentOutput, ctx: &mut ViewContext<Self>) {
        ...
        let autonomy_allowed = is_agent_mode_autonomy_allowed(ctx);
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            AIBlockAction::ViewScreenshot { action_id: _ } => {
                // Computer Use has been removed; the screenshot lightbox is no longer available.
            }
