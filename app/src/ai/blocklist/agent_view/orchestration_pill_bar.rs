...
use warpui::elements::{
    ...
    Container, CornerRadius, CrossAxisAlignment, DEFAULT_UI_LINE_HEIGHT_RATIO, Element, Empty,
...
use crate::ui_components::icon_with_status::{
    self, IconWithStatusVariant, render_icon_with_status,
};
...
fn render_hover_card(
    ...
    // Harness chip: defaults to Warp Agent (Oz) when the orchestration
    // harness hasn't been set yet so the chip slot stays useful for
    // in-progress local conversations. The brand color matches
    // `harness_display` (e.g. orange for Claude Code, blue for Gemini CLI).
