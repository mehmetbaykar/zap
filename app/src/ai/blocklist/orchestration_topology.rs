...
// Zap: the fork has not adopted upstream's drill-down pill bar, so this has
// no caller yet. Kept in sync with upstream rather than deleted.
#[allow(dead_code)]
...
/// descendants.
// Zap: the subtree badge that consumes this lives in upstream's pill bar,
// which the fork has not adopted; only tests read these fields today.
#[allow(dead_code)]
#[derive(Clone, Debug, PartialEq, Eq)]
