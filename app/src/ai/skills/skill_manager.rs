...
use ai::skills::{
    get_provider_for_path, provider_rank, ParsedSkill, SkillProvider, SkillReference,
};
...
impl SkillManager {
    ...
    pub fn get_skills_for_working_directory(
        ...
        let path_matches_location = |path: &LocalOrRemotePath| match (working_directory, path) {
            ...
            (None, LocalOrRemotePath::Local(_)) => false,
    ...
    pub fn find_skill_by_name(&self, name: &str) -> Option<&ParsedSkill> {
        ...
        self.bundled_skills.local().skill_by_name(name)
    }
