//! Agent management dashboard view: a full-view list of local interactive conversations and
//! ambient agent runs, with filtering and a details side panel.
//!
//! Zap adaptations from upstream (see the port report for full rationale):
//! - No cloud setup guide: `cloud_setup_guide_view` is not ported (skip-listed). Both the
//!   zero-state (no items) and the explicit "Get started" toggle render a plain local empty
//!   state instead.
//! - No `HarnessAvailabilityModel`: the harness filter is built from a fixed list of this fork's
//!   harnesses rather than a dynamic "available harnesses" query, and is always shown.
//! - No server-side task fetching: `AgentConversationsModel` here has no `fetch_tasks_for_filters`
//!   (upstream's cloud fetch-on-filter-change); filter changes just recompute from the local
//!   model. `AgentConversationsModel::is_loading()` always returns `false` in this fork (there is
//!   no polling), so the loading state/icon are effectively dead but kept for structural parity.
//! - `AgentSource::CloudMode` does not exist in this fork's `AgentSource` (deleted outright, not
//!   stubbed, per this fork's own established pattern) — dropped from the source filter list.
//! - `AgentType::Cloud` does not exist (see `agent_type_selector.rs`) — "New agent" always spawns
//!   a local agent.
//! - `cancel_task_with_toast` (a cloud RPC) does not exist; cancelling a task shows a toast
//!   without contacting a server. In practice this path is unreachable: ambient tasks are never
//!   populated outside tests (see `AgentConversationsModel`'s doc comment).
//! - Plan-artifact clicks dispatch `WorkspaceAction::OpenAIDocumentPane` directly instead of
//!   bubbling a `NotebookId`-based event (matches `crate::notifications::item_rendering`).
...
/// Fixed harness list for the harness filter dropdown. Zap has no `HarnessAvailabilityModel`
/// (which upstream uses to compute "installed/available harnesses"); every harness this fork
/// knows about is always offered as a filter option. `Harness::Unknown` is intentionally excluded
/// (nothing user-facing should ever filter for "unknown").
const FILTERABLE_HARNESSES: [Harness; 4] = [
    Harness::Oz,
    Harness::Claude,
    Harness::OpenCode,
    Harness::Gemini,
...
pub struct AgentManagementView {
    ...
    /// Whether the user is viewing the empty-state / getting-started guide (toggled via button,
    /// or forced on whenever there are zero items).
...
enum ViewState {
    /// Showing the empty state (this is the zero state when there are no items, or the user
    /// toggled "Get started")
    EmptyState { has_items: bool },
...
impl AgentManagementView {
    pub fn new(
        ...
        let all_filter_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("All", NakedTheme)
                ...
                .with_tooltip(crate::t!("agent-management-filter-all-tooltip"))
                .on_click(|ctx| {
        ...
        let personal_filter_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(crate::t!("agent-management-filter-personal"), NakedTheme)
                .with_size(ButtonSize::Small)
                .with_tooltip(crate::t!("agent-management-filter-personal-tooltip"))
                .on_click(|ctx| {
                    ctx.dispatch_typed_action(AgentManagementViewAction::SetOwnerFilter(
        ...
        let setup_guide_button = CompactibleActionButton::new(
            crate::t!("agent-management-get-started").to_string(),
            None,
            ButtonSize::Small,
        ...
        let view_agents_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(crate::t!("agent-management-view-agents"), NakedTheme)
                .with_size(ButtonSize::Small)
        ...
        let no_filter_results_button = ctx.add_typed_action_view(move |_ctx| {
            ActionButton::new(crate::t!("agent-management-clear-filters"), SecondaryTheme)
                .with_size(ButtonSize::Small)
                .on_click(move |ctx| {
        ...
        let clear_all_filters_button = ctx.add_typed_action_view(move |_ctx| {
            ActionButton::new(crate::t!("agent-management-clear-all"), NakedTheme)
                .with_icon(Icon::X)
                .with_size(ButtonSize::Small)
                .on_click(move |ctx| {
                    ctx.dispatch_typed_action(AgentManagementViewAction::ClearFilters)
        ...
        let new_agent_button = CompactibleActionButton::new(
            crate::t!("agent-management-new-agent").to_string(),
            None,
            ButtonSize::Small,
    ...
    fn get_view_state(&self, app: &AppContext) -> ViewState {
        ...
        if !has_items || self.is_viewing_setup_guide {
            return ViewState::EmptyState { has_items };
        }
    ...
    /// Get current filters for persistence. Zap does not currently wire this into window
    /// save/restore (see `Workspace::window_snapshot`'s `agent_management_filters: None`); kept
    /// as public API so a future persistence pass has a stable entry point, matching the
    /// pre-existing `PersistedAgentManagementFilters`/`crates/persistence` schema, which already
    /// has a column for it.
    ...
    fn create_status_dropdown(
        ...
        Self::setup_filter_menu(&mut dropdown, &crate::t!("agent-management-status"), ctx);
    ...
    fn create_source_dropdown(
        ...
        let mut dropdown = Dropdown::new(ctx);
        Self::setup_filter_menu(&mut dropdown, &crate::t!("agent-management-source"), ctx);
    ...
    /// Build the list of source filter items. Zap's `AgentSource` has no `CloudMode` variant
    /// (deleted, not stubbed); every other variant is always offered (there is no
    /// `ScheduledAmbientAgents`/`InteractiveConversationManagementView`-style availability gate
    /// on the source list itself in this fork beyond what already gates whether entries with
    /// that source can ever exist).
    ...
    /// Update the source dropdown items when tasks change. Kept for structural parity with
    /// upstream (which recomputes based on what sources are actually present); the fork's list is
    /// currently static, but this keeps the selected-item highlight in sync with `self.filters`.
    ...
    fn create_created_on_dropdown(
        ...
        let items = vec![
            ...
            MenuItem::Item(
                MenuItemFields::new(crate::t!("agent-management-last-24-hours"))
            ...
            MenuItem::Item(
                MenuItemFields::new(crate::t!("agent-management-past-3-days"))
                    .with_on_select_action(DropdownAction::select_action_and_close(
                        AgentManagementViewAction::SetCreatedOnFilter(CreatedOnFilter::Past3Days),
                    )),
            ),
            MenuItem::Item(
                MenuItemFields::new(crate::t!("agent-management-last-week")).with_on_select_action(
                    DropdownAction::select_action_and_close(
    ...
    fn create_artifact_dropdown(
        ...
        Self::setup_filter_menu(
            &mut dropdown,
            &crate::t!("agent-management-has-artifact"),
            ctx,
        );
        ...
        let items = vec![
            ...
            MenuItem::Item(
                MenuItemFields::new(crate::t!("agent-management-artifact-pull-request"))
                    .with_on_select_action(DropdownAction::select_action_and_close(
                        AgentManagementViewAction::SetArtifactFilter(ArtifactFilter::PullRequest),
                    )),
            ),
            MenuItem::Item(
                MenuItemFields::new(crate::t!("agent-management-artifact-plan"))
                    .with_on_select_action(DropdownAction::select_action_and_close(
                        AgentManagementViewAction::SetArtifactFilter(ArtifactFilter::Plan),
                    )),
            ),
            MenuItem::Item(
                MenuItemFields::new(crate::t!("agent-management-artifact-screenshot"))
                    .with_on_select_action(DropdownAction::select_action_and_close(
                        AgentManagementViewAction::SetArtifactFilter(ArtifactFilter::Screenshot),
                    )),
            ),
            MenuItem::Item(
                MenuItemFields::new(crate::t!("agent-management-artifact-file"))
                    .with_on_select_action(DropdownAction::select_action_and_close(
    ...
    fn create_harness_dropdown(
        ...
        let mut dropdown = Dropdown::new(ctx);
        Self::setup_filter_menu(&mut dropdown, &crate::t!("agent-management-harness"), ctx);

        let items = Self::build_harness_dropdown_items();
        dropdown.set_rich_items(items, ctx);
        dropdown.set_selected_by_index(0, ctx);
        dropdown
    }

    fn build_harness_dropdown_items() -> Vec<MenuItem<DropdownAction>> {
        let mut items = vec![MenuItem::Item(
            MenuItemFields::new("All").with_on_select_action(
                DropdownAction::select_action_and_close(
        ...
        for harness in FILTERABLE_HARNESSES {
    ...
    fn create_environment_dropdown(
        ...
        Self::setup_searchable_filter_menu(
            &mut dropdown,
            &crate::t!("agent-management-environment"),
            ctx,
        );
    ...
    fn create_creator_dropdown(
        ...
        Self::setup_searchable_filter_menu(
            &mut dropdown,
            &crate::t!("agent-management-created-by"),
            ctx,
        );
        dropdown
    }
    ...
    fn setup_searchable_filter_menu<A: DropdownItemAction>(
        ...
        dropdown.set_main_axis_size(MainAxisSize::Min, ctx);
        dropdown.set_menu_header_text_override(move |text| format!("{}: {}", label_prefix, text));
        dropdown.set_button_variant(warpui::ui_components::button::ButtonVariant::Secondary);
    }

    /// Update the available options depending on the most recent set of ambient agent tasks. Zap
    /// has no `AgentConversationsModel::get_all_environment_ids_and_names` (a cloud-fetch-backed
    /// lookup); the environment list is derived directly from whatever tasks are currently held
    /// locally (in practice, an empty list — ambient tasks are never populated outside tests).
    fn update_environment_dropdown(&mut self, ctx: &mut ViewContext<Self>) {
        let model = AgentConversationsModel::as_ref(ctx);
        let mut envs: HashMap<String, String> = HashMap::new();
        for task in model.tasks_iter() {
            if let Some(id) = task
                .agent_config_snapshot
                .as_ref()
                .and_then(|snapshot| snapshot.environment_id.clone())
            {
                envs.entry(id.clone()).or_insert(id);
            }
        }
    ...
    /// Common handler for filter changes: recompute from the local model and persist the app
    /// state (tab layout etc; filter values themselves are not currently round-tripped, see
    /// `get_filters`).
    ...
    fn get_tasks_from_model(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let cards: Vec<CardData> = model
            ...
            .map(|entry| {
                ...
                // Zap has no Warp-hosted session/conversation links (see
                // `AgentConversationsModel::resolve_copy_link`), so this always yields `None`.
    ...
    fn handle_action_buttons_event(
        ...
        match event {
            ...
            AgentDetailsButtonEvent::CancelTask { task_id } => {
                ...
                // Zap has no server to cancel a task on; ambient tasks are never populated
                // outside tests, so in practice this button is never shown.
    ...
    fn handle_conversation_updated(
        ...
        match kind {
            ...
            ConversationUpdateKind::MetadataChanged | ConversationUpdateKind::TitleChanged => {
                self.get_tasks_from_model(ctx)
            }
    ...
    fn handle_agent_type_selector_event(
        ...
        match event {
            AgentTypeSelectorEvent::Selected(AgentType::Local) => {
            ...
            if state.is_hovered() {
                ...
                stack.add_positioned_overlay_child(
                    ...
                    warpui::elements::OffsetPositioning::offset_from_parent(
                        ...
                        warpui::elements::ParentAnchor::BottomMiddle,
    ...
    fn render_card_at_index(&self, index: usize, app: &AppContext) -> Box<dyn Element> {
        let Some(card) = self.items.get(index) else {
            return Empty::new().finish();
        };
        self.render_card(card, app)
    }

    fn render_card(&self, card: &CardState, app: &AppContext) -> Box<dyn Element> {
        let appearance = Appearance::as_ref(app);
        let theme = appearance.theme();
        let model = AgentConversationsModel::as_ref(app);
        let Some(entry) = model.get_entry_by_id(&card.item_id, app) else {
            return Empty::new().finish();
        };

        let (status_icon, status_color) = entry.display.status.status_icon_and_color(theme);
        let title = Text::new_inline(
            entry.display.title.clone(),
        ...
        // Zap-simplified: upstream composites a harness/source icon with a small status badge
        // via `agent_icon`/`icon_with_status` helpers whose fork API has diverged from upstream's
        // (renamed/reshaped during de-clouding); render the status icon alone rather than guess
        // at the current composite-icon signature.
        let status_icon_element = warpui::elements::ConstrainedBox::new(
            status_icon.to_warpui_icon(status_color.into()).finish(),
        )
        .with_width(CARD_AGENT_ICON_SIZE)
        .with_height(CARD_AGENT_ICON_SIZE)
        .finish();
        ...
        if let Some(creator_name) = entry.display.creator.name.as_deref() {
            header_row.add_child(Self::render_avatar_with_tooltip(
                creator_name,
                appearance,
                card.avatar_hover_state.clone(),
            ));
        }

        if let Some(session_status) = &entry.display.session_status {
            header_row.add_child(Self::render_session_status_label(
                appearance,
                card.session_status_hover_state.clone(),
                session_status,
            ));
        }

        let updated_text = Text::new_inline(
            format_approx_duration_from_now_utc(entry.display.last_updated),
            appearance.ui_font_family(),
        ...
        let mut card_column = Flex::column()
            ...
            .with_child(metadata_row.finish());

        if let Some(artifact_buttons) = &card.artifact_buttons_view {
            card_column.add_child(ChildView::new(artifact_buttons).finish());
        }

        let card_content = Container::new(card_column.finish())
            .with_uniform_padding(12.)
            .finish();

        let action_buttons = ChildView::new(&card.action_buttons_view).finish();
        let stack = Stack::new().with_child(card_content).with_positioned_child(
            action_buttons,
            warpui::elements::OffsetPositioning::offset_from_parent(
    ...
    fn render_empty_state(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let title = Text::new_inline(
            crate::t!("agent-management-empty-state-title"),
            appearance.ui_font_family(),
        ...
        let description = Text::new_inline(
            crate::t!("agent-management-empty-state-description"),
            appearance.ui_font_family(),
            appearance.ui_font_size(),
        )
        .with_color(theme.nonactive_ui_text_color().into())
    ...
    fn render_header(&self, app: &AppContext) -> Box<dyn Element> {
        match self.get_view_state(app) {
            ViewState::EmptyState { has_items } => self.render_setup_guide_header(has_items, app),
    ...
    fn render_task_list_header(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let build_header = |use_expanded: bool| {
            ...
            let mut filters_wrap = Wrap::row()
                ...
                .with_child(ChildView::new(&self.harness_dropdown).finish())
                .with_child(ChildView::new(&self.environment_dropdown).finish());
    ...
    fn render_search_input(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let search_row =
            warpui::elements::Clipped::new(ChildView::new(&self.search_editor).finish()).finish();
...
impl View for AgentManagementView {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        let main_content = match self.get_view_state(app) {
            ViewState::EmptyState { .. } => self.render_empty_state(app),
