...
impl ServerFileBrowserView {
    ...
    pub fn set_remote_root(
        ...
        // When the host or session changes, the old progress polling is no longer relevant to the new connection;
        // stop it first to avoid redundant timer callbacks. If new upload/download tasks are created during
        // subsequent loading, `schedule_progress_poll` will be restarted.
    ...
    match response.result {
        Some(list_directory_response::Result::Success(success)) => {
            ...
            for entry in success.entries {
                // Compatibility with older servers: the `kind` field was introduced by the new protocol; the
                // `FileSystemEntryKind::Unspecified` returned by old servers cannot be directly downgraded to `Other`,
                // otherwise double-clicking a directory would mistakenly go through the ResolvePath (unsupported by old servers) flow and error out.
                // Here we fall back to the old `is_dir` field when `Unspecified`; an unknown kind (a variant a future server
                // might add) is classified as `Other`, to avoid misjudgment from guessing based on `is_dir`.
