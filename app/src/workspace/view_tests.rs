...
pub(crate) fn initialize_app(app: &mut App) {
    ...
    let _ =
        warp_ssh_manager::set_database_path(std::env::temp_dir().join("warp_view_tests.sqlite"));
    ...
    app.add_singleton_model(crate::ai::outline::RepoOutlines::new_for_test);
...
fn test_open_file_with_target_event_preserves_requested_line() {
    ...
    App::test((), |mut app| async move {
        ...
        // Zap: upstream's copy of this test omits FileModel because its open-file path
        // never reaches the file notebook. This fork's does, so the singleton has to
        // exist or the run panics with "Cannot get singleton model ... never registered".
...
fn test_open_markdown_viewer_target_preserves_requested_line() {
    App::test((), |mut app| async move {
        initialize_app(&mut app);
        // Zap: see the note in test_open_file_with_target_event_preserves_requested_line —
        // this fork's open-file path reaches the file notebook, which needs FileModel.
        app.add_singleton_model(warp_files::FileModel::new);
        let workspace = mock_workspace(&mut app);
        let temp_dir = TempDir::new().expect("failed to create temp dir");
...
#[test]
fn test_close_active_horizontal_tab_activates_tab_to_right_earlier_case() {
    let _vertical_tabs_guard = FeatureFlag::VerticalTabs.override_enabled(true);
    App::test((), |mut app| async move {
        initialize_app(&mut app);
        app.update(|ctx| {
...
#[test]
fn test_close_last_horizontal_tab_activates_tab_to_left_earlier_case() {
    let _vertical_tabs_guard = FeatureFlag::VerticalTabs.override_enabled(true);
    App::test((), |mut app| async move {
        initialize_app(&mut app);
        app.update(|ctx| {
