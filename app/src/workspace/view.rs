...
#[cfg(feature = "local_fs")]
fn buffer_location_for_path(
    path: &LocalOrRemotePath,
) -> crate::code::buffer_location::BufferLocation {
    match path {
        LocalOrRemotePath::Local(path) => {
            crate::code::buffer_location::BufferLocation::Local(path.clone())
        }
        LocalOrRemotePath::Remote(remote) => crate::code::buffer_location::BufferLocation::Remote(
            crate::code::buffer_location::RemotePath::new(
...
pub struct Workspace {
    ...
    /// Tab pane-group ids ordered most-recently-activated first. Used to drive
    /// `CtrlTabBehavior::CycleMostRecentTab`'s Ctrl+Tab palette (see `tab_navigation_data`).
...
impl Workspace {
    ...
    pub fn new(
        ...
        // Refresh UI after the CLI agent install scan completes, including the new-tab menu and titlebar buttons.
        ctx.subscribe_to_model(
    ...
    /// Builds the tab list backing the Ctrl+Tab palette when
    /// `CtrlTabBehavior::CycleMostRecentTab` is active, ordered most-recently-activated first.
    ...
    pub fn open_file_with_target(
        ...
        match target {
            ...
            FileTarget::ImageViewer(_layout) => {
                // Images always open in a new tab; see `insert_image_pane`. The layout carried by target is intentionally ignored here.
    ...
    fn handle_left_panel_event(&mut self, event: &LeftPanelEvent, ctx: &mut ViewContext<Self>) {
        match event {
            ...
            LeftPanelEvent::OpenFileWithTarget {
                ...
                crate::code::buffer_location::BufferLocation::Remote(remote_path) => {
                    #[cfg(feature = "local_tty")]
            ...
            RightPanelEvent::OpenFileInNewTab {
                ...
                LocalOrRemotePath::Remote(path) => {
                    ...
                    self.open_remote_file_with_target(
                        crate::code::buffer_location::RemotePath::new(path.host_id, path.path),
                        line_and_column,
                        ctx,
                    );
                    #[cfg(not(feature = "local_tty"))]
    ...
    fn unified_new_session_menu_items(
        ...
        // 5. Coding Agents - only installed agents with tab_menu enabled appear in the menu
            ...
            pane_group
                ...
                .find(|(pane_id, file_view)| {
                    !pane_group.as_ref(ctx).is_pane_hidden_for_close(*pane_id)
                        && file_view.as_ref(ctx).local_path().as_deref() == path.to_local_path()
                })
    ...
    fn open_remote_image(
        ...
        // Create an empty pane with a loading spinner first, then fill it asynchronously with bytes.
        ...
        let context_data: Option<(
            ...
            read_result.and_then(
                |(repo_path, terminal_view): (
                    ...
                    let diff_state_model = repo_path.as_ref().and_then(|repo_path| {
                        self.working_directories_model.update(ctx, |model, ctx| {
                            model.get_or_create_diff_state_model(
                                buffer_location_for_path(repo_path),
                                ctx,
                            )
                        })
                    })?;
    ...
    fn open_code_review_panel_from_arg(
        ...
        let diff_state_model = repo_location.as_ref().and_then(|repo_path| {
            self.working_directories_model.update(ctx, |model, ctx| {
                model.get_or_create_diff_state_model(buffer_location_for_path(repo_path), ctx)
            })
        });
        ...
        let context = read_result.and_then(
            |(repo_path, terminal_view): (
                ...
                let diff_state_model = repo_path.as_ref().and_then(|rp| {
                    self.working_directories_model.update(ctx, |model, ctx| {
                        model.get_or_create_diff_state_model(buffer_location_for_path(rp), ctx)
                    })
                })?;
        ...
        match new_tab_placement_setting {
            ...
            _ => {
                if self.tab_count() == 0 {
                    ...
                    self.tab_mru_order
                        .push(self.tabs.last().expect("just pushed").pane_group.id());
                    self.activate_tab_internal(self.tab_count() - 1, ctx);
                } else {
                    self.tabs
                        .insert(self.active_tab_index + 1, TabData::new(new_pane_group));
                    self.tab_mru_order
                        .push(self.tabs[self.active_tab_index + 1].pane_group.id());
            ...
            if should_open_in_new_tab {
                ...
                workspace.add_new_session_tab_with_default_mode(
                    ...
                    Some(ConversationRestorationInNewPaneType::Forked {
                        ...
                        has_initial_query: summarize_after_fork || initial_prompt.is_some(),
                    }),
                    false,
                    ctx,
                );
        ...
        terminal_view.update(ctx, |terminal, ctx| {
            ...
            if let Some(prompt) = initial_prompt {
                terminal.send_user_query_after_next_conversation_finished(
                    ...
                    /* show_send_now_button */ false, /* locked_for_pending_lrc */ false,
                    ctx,
                );
            }
        });
        ...
        match event {
            ...
            CommandPaletteEvent::OpenFile {
                ...
                #[cfg(feature = "local_fs")]
                {
                    ...
                    let code_source = match location {
                        LocalOrRemotePath::Local(path) => CodeSource::CommandPalette { path },
                        LocalOrRemotePath::Remote(remote) => CodeSource::RemoteFileTree {
    ...
    fn refresh_working_directories_for_pane_group(
        ...
        let terminal_cwds: Vec<(EntityId, LocalOrRemotePath)> = pane_group
            ...
            .filter_map(|(id, cwd)| {
                cwd.map(|path| (id, LocalOrRemotePath::Local(path.into())))
            })
            .collect();
    ...
    pub(crate) fn handle_file_tree_event(
        ...
        match event {
            ...
            pane_group::Event::InsertCodeReviewComments {
                ...
                let repo_path = LocalOrRemotePath::Local(repo_path.clone());
            ...
            if has_remote_server {
                if let (Some(sid), Some(cwd)) = (session_id, remote_pwd.clone()) {
                    RemoteServerManager::handle(ctx).update(ctx, |mgr, ctx| {
                        mgr.navigate_to_directory(sid, cwd.display_path(), ctx);
                    });
                }
            }
    ...
    fn render_tab_bar_contents(
        ...
        if vertical_tabs_active {
            ...
            // Titlebar agent quick-launch buttons in vertical tab-bar mode
        ...
        // Titlebar agent quick-launch buttons in horizontal tab-bar mode
        for button in self.render_cli_agent_titlebar_buttons(appearance, ctx) {
            tab_bar.add_child(button);
        }
    ...
    /// Render titlebar quick-launch buttons for installed CLI agents.
    /// Only show agents whose per-agent titlebar setting is true.
        ...
        for agent in enum_iterator::all::<CLIAgent>() {
            ...
            let button =
                icon_button_with_color(appearance, icon, false, handle.clone(), icon_color)
                    ...
                    // Shrink the icon to 14x14: increase padding from 4 to 5.0 while keeping the 24x24 button frame unchanged.
