...
fn test_disabled_automatic_updates_skip_background_requests() {
    App::test((), |mut app| async move {
        ...
        app.update_model(&autoupdate_state, |autoupdate, ctx| {
            ...
            // The merged autoupdate model defers all requests until polling has
            // started (post-onboarding); flip it on so the disabled-setting
            // drain semantics below are what's actually under test.
