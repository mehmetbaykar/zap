...
/// 2. If the asset has no digest field (old release), skip;
/// 3. If the digest is not in `sha256:xxx` format or has the wrong length, skip;
/// 4. In the normal case, stream-read the file to compute sha256; on a match return Ok, on a
///    mismatch return Err and mark the file as invalid (the caller decides whether to delete it).
///
/// This function should not be called outside the OSS path: other channels use codesign / Inno's
...
pub(crate) fn verify_oss_asset_sha256(
    ...
    let Some(asset) = release.find_asset(asset_name) else {
        log::warn!(
            "openWarp: {asset_name} not found in the cached release, skipping SHA-256 verification"
        );
        return Ok(());
    };
...
/// Download progress, shared across platforms. `total` is `None` when the server does not return a
/// `Content-Length`.
#[derive(Clone, Debug)]
...
/// The platform download functions push progress back to the UI thread through this callback.
...
pub struct AutoupdateState {
    ...
    /// The latest progress of the download currently in flight; cleared by this module when Stage
    /// leaves DownloadingUpdate. The UI reads it via `download_progress()` and renders the progress
    /// bar.
...
impl AutoupdateState {
    ...
    /// The current download progress; only meaningful while the stage is DownloadingUpdate.
    ...
    pub fn register(ctx: &mut AppContext, http_client: Arc<http_client::Client>) {
        ctx.add_singleton_model(move |ctx| {
            ...
            {
                // Start the polling loop; whether a background check is actually issued is
    ...
    fn should_update(&mut self, version: VersionInfo, update_id: String) -> UpdateReady {
        ...
        // openWarp note: `ChannelState::app_version()` returns the original injected
        // `GIT_RELEASE_TAG` value, which **keeps** the `v` prefix (e.g. `v2026.05.10.preview`),
        // whereas `VersionInfo.version` on the OSS branch is provided by
        // `github::GithubRelease::version()`, which has already `trim_start_matches('v')` to remove
        // the `v` (e.g. `2026.05.10.preview`). A direct string equality comparison would always be
        // false, causing "already up to date" to be misjudged as "new version found". Here we do an
        // idempotent prefix normalization:
        // - official Warp: tags always have `v`, so both sides remain equal after trimming, behavior
        //   unchanged.
        // - openWarp: they only become equal after trimming, correctly recognizing the same version.
    ...
    fn on_update_check_complete(
        ...
        match &update_available {
            Ok(UpdateReady::CanDownload {
                ...
                // openWarp (Channel::Oss): uses the same download flow as the official build; the
                // platform download_update_and_cleanup internally picks the appropriate asset and
                // skips codesign verify on the OSS branch.
    ...
    fn download_new_update(
        ...
        // Initialize to 0/unknown when entering the download stage, so the UI immediately sees
        // "downloading" feedback; below, the actual progress of each chunk is pushed back via mpsc
        // to overwrite it.
        ...
        // Progress channel: the download future pushes DownloadProgress to tx, and the
        // spawn_stream_local on the model's main thread receives it, writes self.download_progress,
        // and notifies.
        // Use an unbounded channel to avoid the download blocking on send; the UI only reads the
        // latest progress each time, and intermediate backlog is overwritten directly when the model
        // processes it, so there is no "frame stall".
        ...
        let on_progress: ProgressCallback = Arc::new(move |p| {
            // Ignore when the receiver is disconnected (download finished / model destroyed); does
            // not affect the download itself.
            let _ = progress_tx.unbounded_send(p);
        });

        // Beware of a race: after the ctx.spawn future completes, the main thread immediately runs
        // the callback; at that point progress_rx may still have buffered items not yet consumed by
        // spawn_stream_local. If the callback crudely sets `download_progress = None`, the stream's
        // subsequent on_item would change None back to Some, producing "ghost progress". Fix: on_item
        // only writes while the stage is still DownloadingUpdate; stale items are discarded
        // automatically. When the stream ends naturally (all tx Arcs dropped), on_done clears the
        ...
        let _ = ctx.spawn(
            ...
            move |autoupdate_state, download_ready, ctx| {
                // Don't clear download_progress here: leave it to spawn_stream_local's on_done to
                // clean up uniformly (to avoid the race, see the comment above).
...
async fn fetch_version(
    ...
    // openWarp uses GitHub Releases (mehmetbaykar/zap), completely bypassing official Warp's
    // channel_versions / GCS. Return early to avoid the subsequent fetch_channel_versions
    // inevitably failing.
...
pub fn apply_update(
    ...
    // openWarp: apply_update on all three platforms uses the same entry point as the official
    // build; each platform's relaunch() just selects a different "install action" for the OSS branch
    // (mac: open dmg / Windows: non-silent Inno install / Linux AppImage: in-place overwrite +
    // restart).
...
{
    // Non-macOS OSS channels apply their update in place during download (Linux
    // overwrites the AppImage; Windows runs the installer on relaunch), so there's
    // nothing to finalize here. macOS now performs a verified silent bundle swap
    // via apply_update_async, so it falls through to the platform path below.
