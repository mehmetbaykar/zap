...
#define MyAppURL "https://zap.zerx.dev/"
  ...
  ; By default follows the official channel's `dev.warp.*` naming; for OSS, bundle.ps1 overrides it to `dev.zap.Zap`.
...
; For OSS, bundle.ps1 passes `InnoAppId=zap-oss`; other channels use the default `warp-terminal-{ReleaseChannel}`.
#ifndef InnoAppId
...
Root: HKCU; Subkey: "SOFTWARE\Zap.dev\{#MyAppName}"; ValueType: string; ValueName: "InstallationPath"; ValueData: "{app}\{#MyAppExeName}"; Flags: uninsdeletevalue
; App Paths registration lets Win+R and other apps resolve the executable by name.
