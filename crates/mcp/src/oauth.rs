...
pub async fn make_authenticated_client(
    ...
    if auth_manager.initialize_from_store().await? && auth_manager.get_access_token().await.is_ok()
    {
        if let (Some(client_id), Some(client_secret)) = (client_id, client_secret) {
            // `configure_client` requires the manager's OAuth server metadata to already be
            // populated (so future automatic token refreshes have a client secret to use).
            // The rmcp version we're pinned to has no public setter for that field, so we
            // reuse `initialize_from_store` to discover and populate it from the credential
    ...
    // Attempt dynamic client registration via the standard OAuth flow first; this also
    // discovers the authorization server's metadata as a side effect of
    // `start_authorization`, which the rmcp version we're pinned to only exposes through
    // this path (there is no public setter for the manager's metadata field).
    ...
    // With DCR, we don't pass in explicit scopes; they are specified during dynamic
    // registration. For apps for which we have static client IDs (e.g. GitHub), manually
    // override both the scopes and the client configuration below.
    let mut scopes: &[&str] = &[];

    // TODO(vorporeal): adjust APIs in rmcp so that we don't need this redundant
    // discover_metadata() call (it's already made within start_authorization() above, but
    // we can't observe the result from outside the crate).
    if let Some(provider) = auth_manager
        .discover_metadata()
        .await?
        .issuer
        .as_deref()
