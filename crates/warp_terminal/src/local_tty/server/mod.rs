//! Logic relating to a secondary process which Zap uses to spawn new shell
//! sessions.
//!
...
//! early during Zap's initialization, we can be more confident that there are
...
pub fn run_terminal_server(
    args: &warp_cli::TerminalServerArgs,
    init_feature_flags: impl FnOnce(),
    init_platform: impl FnOnce() -> anyhow::Result<()>,
) {
...
/// between processes, enabling the Zap application process to communicate
