...
use crate::{
    ...
    AnyModelHandle, AnyView, ApplicationBundleInfo, Clipboard, CurrentRenderWindowGuard,
    CursorInfo, Effect, Element, Entity, EntityId, EntityIdMap, EntityIdSet, Event,
...
impl AppContext {
    ...
    /// (from the window's presenter parent map — this fork's equivalent of
    /// upstream's backend-neutral view hierarchy) and expanded through both
    /// the structural parent/child graph and each view's
    /// [`View::child_view_ids`]. Walking `child_view_ids` is what captures
    /// views that the render-time graph misses: model-held views and child
    /// handles that aren't laid out in the current frame (e.g. a hidden
    /// window that never rendered).
    ///
