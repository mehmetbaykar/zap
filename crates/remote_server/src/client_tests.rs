...
async fn disconnected_on_closed_stream() {
    ...
    let (client_read, client_write) = tokio::io::split(client_stream);
    let executor = executor::Background::default();
    let (client, disconnect_rx, _host_rx) =
        RemoteServerClient::new(client_read.compat(), client_write.compat_write(), &executor);
...
async fn delete_file_round_trip() {
    let (client, _disconnect_rx, _executor) = setup_mock_client(|msg| {
        let host_scoped_request::Message::DeleteFile(req) = unwrap_host_scoped(msg) else {
            panic!("Expected DeleteFile");
        };
        assert_eq!(req.path, "/tmp/old.txt");
    ...
    client
        .delete_file("/tmp/old.txt".to_string())
        .await
        .unwrap();
}

#[tokio::test]
async fn save_buffer_round_trip() {
    let (client, _disconnect_rx, _executor) = setup_mock_client(|msg| {
        let host_scoped_request::Message::SaveBuffer(req) = unwrap_host_scoped(msg) else {
            panic!("Expected SaveBuffer");
        };
        assert_eq!(req.path, "/tmp/buffer.txt");
    ...
    let response = client
        .save_buffer("/tmp/buffer.txt".to_string())
        .await
        .unwrap();
    assert!(matches!(
        response.result,
