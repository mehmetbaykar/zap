...
//! creates the transcript-capable terminal session.
...
/// Holds the live TUI driver and terminal manager.
struct TuiSession {
...
fn init(ctx: &mut AppContext) {
    ...
    match spawn_tui_driver(ctx, window_id, root.clone()) {
        Ok(driver) => {
            ...
            create_terminal_session(&session, &root, &banner, ctx);
        }
        Err(error) => {
...
/// Creates and retains the terminal manager.
    ...
    let manager = LocalTtyTerminalManager::<TuiTerminalSessionView>::create_tui_model(
        ...
        move |surface_init, ctx| {
            let surface = root.update(ctx, |root, ctx| {
                ...
                // Re-render the root so it swaps the startup placeholder for the session.
                ctx.notify();
                surface
            });
            TerminalSurfaceResult {
