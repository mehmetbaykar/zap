//! Local terminal-session TUI surface.
use std::borrow::Cow;
...
use warp::tui_export::{
    ...
    AgentConversationsModel, AgentInteractionMetadata, AgentViewController, AgentViewEntryOrigin,
    AmbientAgentViewModel, BlockId, BlocklistAIActionModel, BlocklistAIContextModel,
    ...
    ChangelogModelEvent, ChangelogRequestType, LoadedConversationData, CommandExecutionSource,
    ConversationFileExport, ConversationSelection, ConversationSelectionHandle,
    EphemeralMessageModel, ExecuteCommandEvent, GitRepoModels, GitRepoStatusModel,
...
/// The local terminal/session surface rendered inside [`RootTuiView`].
...
impl TuiTerminalSessionView {
    ...
    pub(crate) fn new(
        ...
        // when the locally configured model catalog changes, or when
    ...
    fn render_footer(&self, ctx: &AppContext) -> TuiFlex {
        ...
        // Zap: upstream renders a credits/cost usage entry here; the fork has
        // no Warp usage reporting, so the footer goes straight to git stats.
    ...
    /// (Re)subscribes the footer's git status model when the active session's
    /// repository changes.
    ...
    fn handle_accepted_conversation(
        ...
        let target = match (
            ...
            // Zap: server-token-only entries cannot be restored without Warp's
            // cloud storage; treat them like unavailable conversations.
