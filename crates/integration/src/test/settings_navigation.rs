...
//!
//! The fork's sidebar starts with the Agents umbrella and has no cloud
//! Account/Billing pages, so the fixtures below use `About` as the neutral
//! top-level page and the Code umbrella as the neighbour below Agents.
...
///
/// The Agents umbrella is the first row in the fork's sidebar, so this starts
/// from the last row and relies on the nav order wrapping around.
...
pub fn test_settings_keyboard_navigation_up_into_collapsed_umbrella() -> Builder {
    new_builder()
        .with_step(wait_until_bootstrapped_single_pane_for_tab(0))
        // The Code umbrella sits directly below the Agents umbrella, so its
        // first subpage is the row right after the Agents group.
...
pub fn test_settings_search_filters_top_level_pages() -> Builder {
    new_builder()
        ...
        // The starting page no longer matches, so the selection follows the
        // filter onto the surviving page.
