                    ...
                    let expected = pane_group.read(app, |pane_group, ctx| {
                        pane_group
                            ...
                            .find_map(|code_view| {
                                ...
                                // Zap: upstream reads `display_path()`, added alongside its
                                // LocalOrRemotePath rewrite of `path_from_focused_pane`. This fork
                                // kept its own local_fs-gated implementation, so the equivalent
                                // here is BufferLocation::local_path.
                                code_view
                                    .tab_at(code_view.active_tab_index())
                                    .and_then(|tab| tab.location())
                                    .and_then(|path| path.local_path())
