...
fn expanding_ignored_directory_preserves_default_context_filtering() {
    VirtualFS::test("ignored_directory_context", |dirs, mut vfs| {
        ...
        let root = block_on(Entry::build_tree(
            &repo_local,
            &mut Vec::new(),
            &mut gitignores,
            None,
        ...
        App::test((), |mut app| async move {
            ...
            for directory in [None, Some(&build), Some(&nested)] {
                if let Some(directory) = directory {
                    ...
                    await_build_tasks_for_repo(&mut app, &model_handle, &repo).await;
                }
                model_handle.read(&app, |model, _ctx| {
                    let contents = model
...
fn test_index_lazy_loaded_home_dir_succeeds() {
    ...
    App::test((), |mut app| async move {
        ...
        // The first-level build runs asynchronously; wait for it to apply.
