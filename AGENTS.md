...
- **Wave 2026-08-19 — `7d93fa468..04a7f8342`, 110 commits, 2 slices** (`7f398b568` 59 commits, `5f5a739e4` 51 commits). Triage 38 TAKE / 47 STRIP / 13 SPLIT / 12 ADAPT. Gates green both slices (slice 2: 7832/7832). `script/upstream-strip.list` gained 45 verified paths (REMOTE-2111 checkpoint pipeline, orchestration unified stack M1/M2, cloud runner display, shared-session manager, teams/environments settings, Factory files skill, `crates/build_cache`); `crates/ai/src/index/full_source_code_embedding` is listed **DEFERRED, not rejected** — a subsystem the fork has never adopted.
  - **Settings split (upstream #15108 + #15122)**: adopted the Code-page split (`code_indexing_page` + `code_editor_review_page`) and the `ai_page.rs` → `warp_agent_page.rs` rename, BYOP UI preserved. **Deliberately not adopted**: `agent_profiles_page.rs` (cloud-subscription profiles, owner decision), and upstream's `knowledge_page.rs` / `cli_agents_page.rs` / `ai_shared.rs` — the fork already ships those surfaces inside `warp_agent_page.rs` via its `AISubpage` mechanism (`SettingsSection::{Knowledge,ThirdPartyCLIAgents}` predate the merge), with full i18n and a fork-invented per-agent chip section, and upstream's versions hardcode English + `docs.warp.dev` and re-add a deliberately-removed Warp-Drive-as-context toggle. Adopting them would have been a regression; restructuring the fork's superset into upstream's file layout is an optional standalone refactor, not merge work.
  - Security scan per slice (new endpoints / credentials / personal data): clean. Upstream's `FactoryMcp` flag was dropped — enabling it attaches a Warp-hosted MCP server authenticated with a logged-in session token.
- **Operational gotchas (hard-won — keep)**:
  ...
  10. **A `merge=openwarp-ours` zone silently withholds *wanted* upstream changes** (`.gitattributes`: `app/src/ai/agent_sdk/**`, `agent_view/**`, …). Git keeps ours with no conflict and no notice, so a feature can land half-merged: 2026-08-19 that stranded `skill_dirs_publish.rs` with no `mod` decl, and left `driver.rs` without the new `CLIAgentSessionStatus::Cancelled` arm. After every merge, `git status` the ours-zones for orphan `A ` files and hand-port the half that git refused.
  11. **Upstream changing a type inside a crate we deleted breaks kept call sites.** Upstream moved `Revision` to `crates/cloud_objects` and `DiffMatchFailures` to `crates/ai`; their `Copy`/field changes landed on stripped paths while the matching `.clone()` removals landed in kept files. Only the *test build* catches this — `cargo check -p warp` alone does not.
  12. `rtk`-filtered `git status`/`git log`/`git grep` truncate output. Use `rtk proxy git …` whenever an exact file list drives a decision; a truncated conflict list silently under-resolves the merge.
  13. macOS `xargs` has no `-d`: use `tr '\n' '\0' | xargs -0 …` for the keep-deleted sweep. And a backgrounded `cargo … | tail` reports **tail's** exit code — redirect to a file and echo `$?`, or a failed build reads as success.
