...
- `cargo check` passes.
- `cargo nextest run --no-fail-fast --workspace --exclude command-signatures-v2` currently fails during `cargo test --no-run`, before individual tests execute, because `crates/managed_secrets_wasm` cannot compile: `ManagedSecretValue::openai_api_key(...)` is referenced in `crates/managed_secrets_wasm/src/lib.rs`, but the current `ManagedSecretValue` implementation in `crates/managed_secrets/src/secret_value.rs` does not define that helper/variant. (As of 2026-06-29 `managed_secrets_wasm` is the only remaining build blocker — `cargo test --no-run --workspace --exclude command-signatures-v2 --exclude managed_secrets_wasm` Finishes clean.)

Upstream sync state (2026-06-29):
- Latest triaged upstream/warp (warpdotdev/warp) tip: `ed34ab5e5` (2026-07-08). Last clean cherry-pick baseline was `178fe89bc` (#13097); this sync reviewed all 134 commits after that through `ed34ab5e5`. Applied product fixes where they merged cleanly (or via small local ports); skipped cloud/subscription/billing, TUI-only, tab-groups, orchestration/model-router, VA recording, CI/skills-lock, and commits that conflicted heavily with local BYOP/fork APIs. Resume future warp syncs by listing commits after `ed34ab5e5` and re-triaging conflicted candidates (lifecycle Precmd chain, LRC queue race, dangling-tool tests, /repos lazy git, settings deeplink, etc.).
- upstream/zap (zerx-lab/zap) is merged through `54905881d` (#292 SSH/CLI subagent history terminal snapshots, 2026-07); zap is tracked by **merge**, warp by selective **cherry-pick**.
