...
- Latest cherry-picked upstream/warp (warpdotdev/warp) commit: `178fe89bc` (#13097 "Flip context window circle", 2026-06-28). Everything in warp `master` up to that commit is either applied or deliberately skipped (cloud / remote / onboarding / TUI crate / CI / deps / tab-groups / model-router); resume future warp syncs by listing commits after `178fe89bc` and triaging for local product fixes only.
- upstream/zap (zerx-lab/zap) is merged through `45efa893a` (2026-06-29); zap is tracked by **merge**, warp by selective **cherry-pick**.
