...
- **Stale remote (SSH) file-tree fix (2026-07-19)**: files deleted on a remote host kept showing in the file explorer (clicking → load error), only a full SSH reconnect refreshed. Root causes (inherited upstream limitations, verified identical upstream): only detected *git repos* get the remote-side recursive watcher (plain dirs like `~/.zap` never refresh); the watcher push channel's client apply silently drops unmatched updates with no resync; collapse→expand short-circuits on `loaded()`; `cd` re-entry is deduped (`already_sent`); no TTL, no refresh action, and a failed open triggers nothing. AND the make-or-break discovery (found by pre-implementation deep-verification): the `LoadRepoMetadataDirectory` response is applied with **merge-only semantics** (`remove_entries` hardcoded empty; `insert_child` only adds to `parent_to_child_map`) — so even a re-list would never remove deleted entries. Three-layer fix, **no proto/wire change** (works against already-deployed remote servers): (1) `RepoMetadataUpdate.replace_children_of: Option<StandardizedPath>` — set client-side only by the directory-re-list conversion (from the `dir_path` field servers already send); the apply path prunes existing children absent from an authoritative listing (watcher pushes/snapshots stay merge+explicit-removals); (2) collapse→expand of a *remote* directory now force-re-fetches via new `force_reload_remote_directory` (deliberately NOT wired into `ensure_loaded_path` — its `update_directory_contents` caller loops ALL expanded folders of ALL roots on every local-repo/cwd event and would flood the remote with RPCs; verified server cost per re-list = one `max_depth=1` readdir, 5000-entry cap); (3) failed remote file open now self-heals: `CodeViewEvent::FileLoadFailed{location}` → code_pane (Remote-only) → `ActiveFileEvent::LoadFailed` → file tree re-lists the parent. Expansion state proven preserved (view-side `expanded_folders`; grandchildren maps untouched by a parent re-list). Regression tests: prune/merge/empty-dir triple in `file_tree_update_tests.rs`. on the branch caught one real fork bug the local `-p warp` gate can't see: `warp_core::paths::tests::test_tui_mcp_config_path_is_separate_from_gui` (added by adopted #13744) failed deterministically on macOS — `macos_tui_config_dir_name` rewrites `.warp`→`.warp_cli`, but the fork's Oss channel renames the GUI dir to `.zap`, so the rewrite matched nothing and the TUI config dir collided with the GUI's (shared `.mcp.json`). Fixed in `49f3a1f78`: Oss → `.zap_cli` + a `{gui}_cli` fallback for any future rename; warp_core 88/88. Windows job green (leaky-but-passing tests only). LESSON: CI runs `--workspace` — local gate-4 (`-p warp`) misses sibling-crate tests; check `-p warp_core` (and friends) when paths/channel code changes.
