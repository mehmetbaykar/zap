...
- **Dep pins never taken from upstream**: `warp_multi_agent_api` = mehmetbaykar/warp-proto-apis@`14ab9a71`; `rmcp` = warpdotdev@`c0f65dc`; `reqwest 0.12.28` (+ reqwest-eventsource 0.6); `winit` = chenx-dust fork.
...
- **Merge gates** (all must be green; current state at `v2026.07.19.2`): `cargo check --workspace` → 0; `cargo test --workspace --no-run` → 0; `cargo nextest run -p warp --no-fail-fast` → failing-name diff vs `script/upstream-merge-warp-baseline.txt`, which is **EMPTY** (4904 passed / 0 failed / 9 skipped) — any FAIL is a real regression.
...
- **Operational gotchas (hard-won — keep)**:
  ...
  6. CI runs `--workspace`; the local `-p warp` gate misses sibling-crate tests — also run `-p warp_core` etc. when touching paths/channel code.
  7. `upstream-zap` (zerx-lab/zap, merged through `5d874456a`, tracked by merge) shares the fork's `v*` tag scheme — fetching it without `--no-tags` pollutes local tags.
