...
Builds on the regular Agent Mode queued-prompts panel from `specs/REMOTE-1543/`. This spec moves the remaining local user-facing queued-prompt surfaces onto the new panel UI behind `QueuedPromptsV2`.
## Context
...
These rows are user-managed: interactive drag, edit, and delete, exactly like `QueueSlashCommand` and `AutoQueueToggle` rows. The follow-up has not been submitted at queue time; only the summarize is running. Keeping distinct origins also makes the two local routing paths explicit in model tests.
...
## Parallelization
Single workstream. The helper, two callsite edits, and origin enum changes all sit in a tight ownership boundary (`TerminalView`, `Workspace`, and `QueuedQueryModel`). Sub-agents would create merge churn on the same files without reducing wall-clock time.
