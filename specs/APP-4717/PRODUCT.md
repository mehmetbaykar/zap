...
1. With the queued prompts panel visible, the input buffer completely empty, and the top queued row sendable (see 5), pressing Enter in the terminal input sends the top row immediately. This is identical to clicking that row's Send-now button:
   - An agent prompt row is submitted to the same local conversation target Send-now would use, carrying the row's own queued attachments.
...
5. Enter mirrors the top row's Send-now action. Enter-only conditions (non-empty buffer or CLI-agent rich input open) hide the hint and suppress Enter but do not disable the Send-now buttons.
...
7. Header hint:
   ...
   - The hint is visible exactly when an empty-buffer Enter would send the top row, and hidden otherwise (non-empty buffer, no top row, panel hidden, or any case in 8–10). The hint and the Enter behavior must never disagree.
