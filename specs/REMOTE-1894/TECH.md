# Recover local agent conversations from transient transport failures — tech spec
...
Every agent turn has a provider response stream wrapped by `ResponseStream`, which emits events to `BlocklistAIController`; the controller updates the conversation in `BlocklistAIHistoryModel`, whose status drives the local headless driver and UI.
...
- When a resume *was* scheduled, some consumers still treated it as terminal immediately, and the conversion in `ambient_agents` dropped the structured error (and its resume flag) in favor of the stringly `status_error_message`.
...
- [`app/src/ai/agent_sdk/driver.rs:2793 @ 84276e07`](https://github.com/warpdotdev/warp/blob/84276e0732860798fa49eb7372e6ee90cdf1728b/app/src/ai/agent_sdk/driver.rs#L2793) — local headless-run lifecycle decisions
...
- Everything else → `Fail` (terminal). Recovery eligibility (retry and resume) uses the local `AIApiError` recoverability classification in `app/src/ai/api_error.rs` (I12).
...
Upstream validation used a local `TransportReset` LLM mock; the fork retains the client-side state-machine coverage:
...
- The pre-action retry paths (I2) are covered by the unit matrix plus the request-send-failure path.

Lint/build gates: `cargo fmt`, `cargo clippy -p warp --lib --tests -- -D warnings`, and `cargo check -p warp --lib`.
...
- **Stuck "Reconnecting"**: any path that sets `TransientError` and never delivers a recovery would hang the UI state. Mitigated by the local headless driver's 120s deadline, the one-shot resume contract, and the cancellation flip to `Cancelled`; restore-from-disk degrades it to `Error`.
...
- **Local diagnostics**: synthesized truncations appear as `UnexpectedEof` in local logs instead of the generic EOF fallback.
