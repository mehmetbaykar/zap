...
10. Empty-queue behavior is unchanged: draining an empty queue does nothing. Each conversation keeps an independent queue, so attachments on one conversation's rows never appear on another's.
