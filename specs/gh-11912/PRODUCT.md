...
GitHub: [warpdotdev/warp#11912](https://github.com/warpdotdev/warp/issues/11912). Builds on the v2 queued-prompts panel from [`specs/REMOTE-1543/PRODUCT.md`](../REMOTE-1543/PRODUCT.md).
...
- Work in local Agent Mode without interrupting the running agent.
...
9. A drained command executes through the same local terminal routing as a manually submitted shell command. Because the command came from the queue rather than the current editor buffer, any draft the user is composing in the input is preserved while the command starts and completes.
